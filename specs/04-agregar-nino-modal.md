...
- [x] Inputs: NOMBRE COMPLETO (placeholder `Ej. Martina López`), FECHA DE NACIMIENTO (`dd/mm/aaaa`), SALA (dropdown con `Soles`/`Lunas`/`Estrellas`, default `Soles`), ALERGIAS (placeholder `Ej. Maní, Lactosa`), NOTAS MÉDICAS (textarea, placeholder `Indicaciones, medicación, contactos…`).
...
- [ ] `pnpm exec tsc --noEmit` pasa sin errores.  → ver nota
- [x] No se edita `references/pantallas/agregar-nino.dc.html` ni su `support.js`.
- [ ] Las rutas de SPEC 01 (`/`) y SPEC 02 (`/kids`, `/kids/[id]`) siguen funcionando sin cambios.  → ver nota
...
Notas:
- `tsc --noEmit` falla por un error preexistente en `app/layout.tsx:23` (`TS2304: Cannot find name 'LayoutProps'`) que pertenece al scope de SPEC 01, no de SPEC 04. El error no fue introducido por este spec (`app/layout.tsx` no se modificó). Fuera del scope de SPEC 04; requires corrección en SPEC 01 (reemplazar `LayoutProps<"/">` por `{ children: React.ReactNode }` o el tipo equivalente de App Router).
- Consecuencia: el criterio "Las rutas de SPEC 01 (`/`) y SPEC 02 siguen funcionando sin cambios" también queda marcado `[ ]` por la misma causa. En runtime `next dev`/`next build` podrían tolerar el tipo suelto en `next/font`, pero `tsc --noEmit` estricto no.
